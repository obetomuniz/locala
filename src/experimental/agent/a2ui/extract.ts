...
import { looksLikeA2uiStream } from "./detect";
import { unwrapA2uiFence } from "./detect";
import type { A2uiServerMessage } from "./types";
import { A2uiJsonlBuffer } from "./jsonl";
...
export function feedA2uiReply(text: string): string[] {
  ...
  return [...buf.feed(extractA2uiJsonlLines(text).join("\n") + "\n"), ...buf.flush()];
}
