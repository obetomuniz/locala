import { parseA2uiLine } from "./parse";
import { extractA2uiJsonlLines } from "./extract";
import { parsePlaygroundPayload } from "./constraint";
import { synthesizeA2uiMessages } from "./synthesize";
import type { A2uiComponentNode, A2uiServerMessage } from "./types";
  ...
  try {
    ...
    return {
      surfaceUpdate: {
        surfaceId: "main",
        components: [
          { id: String(o.id), component: o.component as Record<string, unknown> },
        ],
      },
    };
  } catch {
...
export function repairAndParseA2ui(text: string): A2uiServerMessage[] {
  ...
  if (componentBank.size > 0) {
    ...
    if (!componentBank.has("root")) {
      const rootChild = components.find((c) => c.id === "card")?.id ?? components[0]?.id;
      if (rootChild) {
        components.unshift({
          id: "root",
          component: {
