...
import type { AgentEvent, AgentEventOf, AgentTransform } from "./types";
 ...
 */
export function tee<T>(source: AsyncIterable<T>): [
  AsyncIterable<T>,
  AsyncIterable<T>,
] {
  const queueA: T[] = [];
  ...
  async function pump() {
    try {
      ...
      for (const w of waitersA.splice(0)) w({ done: true, value: undefined as never });
      for (const w of waitersB.splice(0)) w({ done: true, value: undefined as never });
    }
  }
 ...
 */
export async function collectText(source: AsyncIterable<AgentEvent>): Promise<string> {
  let buf = "";
  for await (const ev of source) {
